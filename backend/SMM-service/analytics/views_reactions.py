...
def update_workspace_data(workspace_data):
    try:
        ...
        for channel_name, posts in workspace_data.items():
            ...
            for post in posts:
                current_channel_name = post["channel_name"]  # Получаем название канала
                message_id = post["message_id"]  # Получаем идентификатор сообщения
                views, emojis = count_emojis(app, current_channel_name, message_id)
                views_total += views
                emojis_total += emojis

            # Обновляем данные о просмотрах и смайликах для текущего канала
